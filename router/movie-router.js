let {bcrypt,scanDB,checkGlobalUser,putDB,deleteDB} = require('./basicConfig.js/basicConfig')
...
router.get('/:id', async(req, res) => {
  console.log("this test works")
   ...
   if (userFound.length === 0) {
    ...
    let savedAWSMovies = await scanDB("Movie-Application-fav-movies",req.params.id,"userID")
  ...
  router.delete('/:id', async(req, res) => {
      ...
      let selectedAWSMovies = await scanDB("Movie-Application-fav-movies",req.params.id,"id")
  ...
  router.post('/', async (req, res) => {
    ...
    let userFound = await scanDB("Movie-Application-users",req.body.userID,"id")
    userFound = userFound.selected_items
    if (userFound.length === 0) {
    res.status(500).json({"message":"User doesnt exist"})
    return
